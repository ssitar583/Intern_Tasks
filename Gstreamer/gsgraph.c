...
//GST_DEBUG_DUMP_DOT_DIR = "/home/soundaryaa/Documents/Gstreamer/";
//GST_DEBUG_DUMP_DOT_DIR ="/home/Soundaryaa/Documents/Gstreamer"; 
...
int main(int argc,char *argv[]){
	...
	//setenv("GST_DEBUG_DUMP_DOT_DIR" ,"/home/soundaryaa/Documents/Gstreamer/",1); 
	...
	if(!data.pipeline || !data.source ||!data.demux||
	   !data.queue_video||!data.h264parse || !data.avdec_h264 || !data.video_sink ||
	...
	gst_bin_add_many(GST_BIN(data.pipeline),data.source,data.demux,data.queue_video,data.h264parse,data.avdec_h264,data.video_sink,
	...
	if(!gst_element_link_many(data.queue_video,data.h264parse,data.avdec_h264,data.video_sink,NULL)||
	...
	//gchar d = GST_DEBUG_BIN_TO_DOT_DATA(data.pipeline,GST_DEBUG_GRAPH_SHOW_ALL);
	//g_print(d);
	//GST_DEBUG_BIN_TO_DOT_FILE
	
	
	
	GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(data.pipeline,GST_DEBUG_GRAPH_SHOW_ALL,"graph");
...
static gboolean handle_input(CustomData *data) {
    ...
    while (TRUE) {
        if (fgets(input, sizeof(input), stdin) != NULL) {
            ...
            input[strcspn(input, "\n")] = '\0';
	    //GST_DEBUG_BIN_TO_DOT_FILE_TS(data.pipeline,GST_DEBUG_GRAPH_SHOW_ALL,"graph");
            if (g_strcmp0(input, "pause") == 0) {
                g_print("Pausing the pipeline\n");
                gst_element_set_state(data->pipeline, GST_STATE_PAUSED);
                 GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(data->pipeline,GST_DEBUG_GRAPH_SHOW_ALL,"graph_pause");
            } else if (g_strcmp0(input, "play") == 0) {
                g_print("Playing the pipeline\n");
                gst_element_set_state(data->pipeline, GST_STATE_PLAYING);
                 GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(data->pipeline,GST_DEBUG_GRAPH_SHOW_ALL,"graph_Play");
            } else if (g_strcmp0(input, "ready") == 0) {
                g_print("Playing the pipeline\n");
                gst_element_set_state(data->pipeline, GST_STATE_READY);
                 GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(data->pipeline,GST_DEBUG_GRAPH_SHOW_ALL,"graph_ready");
            }
            else if (g_strcmp0(input, "null") == 0) {
                g_print("Setting the pipeline to NULL state\n");
                gst_element_set_state(data->pipeline, GST_STATE_NULL);
                 GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(data->pipeline,GST_DEBUG_GRAPH_SHOW_ALL,"graph_null");
			...
			data->playback_speed  /= speed;
	      	...
	      	send_seek_event (data);
                ...
                g_print("Setting playback speed to %.2f\n", speed);
                 GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(data->pipeline,GST_DEBUG_GRAPH_SHOW_ALL,"graph_speed");
