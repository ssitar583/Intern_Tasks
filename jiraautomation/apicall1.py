...
headers = {
    ...
    "Authorization": "Bearer Mzg4MTk5OTgzODUyOniXDXhH36BSHBdSXJT/JT2Jv+7I"
}
...
for url in repository_urls:
    repository_directory = url.split('/')[-1]
    ...
    with open(git_log_file, "w") as file:
        subprocess.run(['git', 'log'], stdout=file)
    ticket_names = []
    with open(f"../{repo_name}_git_log_output.txt", "r", encoding='utf-8') as file:
        for line in file:
            if commit in line:
                break
    ...
    ticket_names_file = f"../{repo_name}_ticket_names.txt"
    if os.path.exists(ticket_names_file):
        os.remove(ticket_names_file)
    
    with open(ticket_names_file, "w") as file:
        for ticket_name in ticket_names:
            file.write(ticket_name + "\n")

    # Read the ticket names from the file
    with open(f"../{repo_name}_ticket_names.txt", "r") as file:
    ...
    for ticket_id in ticket_ids:
        ...
        if response.status_code == 200:
            ...
            updated_data.to_excel(f"../{repo_name}.xlsx", index=False, header=True)
